        ...
        prodAdv.call("ItemSearch", {
                ...
                for (var i = 0; i < sets.length; i++) {
                    ...
                    // otherwise, all we care about is price
                        ...
                        // we need to determine if the PriceBin gap is high enough to warrant bracketing to begin with
                        // -> Android Apps, for example, generally have PriceBin diffs of $0.01, which is stupid, Amazon
                        // so, for each thing, if the different between it and the next is too small (less than $1)
                        // -> combine the two into one PriceBin
                        // -> or just sum all the diffs/5 < 100 ? then only do one bracket
                        var diffSum = 0;
                        for (var o = 0; o < sets[i].Bin.length; o++) {
                            // for each bin, grab min
                            var params = arrayIfNot(sets[i].Bin[o].BinParameter);
                            if (params.length >= 1) {
                                var minP = params[0].Name === 'MinimumPrice' ? params[0].Value : params[1].Value;
                                // parse
                                console.log(params[0]);
                                minP = parseInt(minP, 10);
                                diffSum += minP;
                            }
                        }
                        console.log(diffSum/sets[i].Bin.length);
